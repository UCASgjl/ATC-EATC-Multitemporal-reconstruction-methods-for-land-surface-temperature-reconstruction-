# Author : GJL
# CreateTime : 2023/8/4
# FileName : similar_pixel_research
...
# 每个研究区的云污染影像LST的重建函数
def LST_calculate(file_LST, array1, array2, array3, array4, min_window_size, max_window_size, expected_similar_pixels):
        ...
        for j in range(0, 500):
            if array1[i][j] != 0:
                ...
                if np.all(window1 == 0):
                    ...
                    recon_array[i][j] = similar_pixel_search(i, j, array1, array2, array3, array4, min_window_size, max_window_size, expected_similar_pixels)
...
# 搜索相似像元
def similar_pixel_search(i_old, j_old, array1, array2, array3, array4, min_window_size, max_window_size, expected_similar_pixels):
    ...
    while window_size <= max_window_size:
        ...
        window4 = array4[row_start:row_end, col_start:col_end]
    ...
    LST_reconstruct = a_b_calculate(i, j, window1, window2, window3, window4, ref_center_pixel, ndvi_center_pixel, final_window_size, final_similar_pixel_count, final_similar_pixel_mask)

    return LST_reconstruct


# 检查相似像素数量，计算残差项
def a_b_calculate(i, j, window1, window2, window3, window4, ref_center_pixel, ndvi_center_pixel, window_size, similar_pixel_count, similar_pixel_mask):
    ...
    window4 = window4 * similar_pixel_mask
    ...
    # 计算ξTsj
    Tsj = window4 - window2  #  T(s,a) - T(s,ATC)
    delta_Ts = np.sum(Tsj * similar_pixel_W_mask)

    # print("残差项：", delta_Ts)
    Ts_rc = ref_center_pixel2 + delta_Ts
    print(ref_center_pixel2, delta_Ts)
...
input_folder = "F:\\SAT\\yulin\\cloudy_img_yulin"
output_folder = "F:\\SAT\\yulin\\cloud_dilation_yulin_nonan"
...
def arr2raster(ref_dataset_file, cloud_file, result_LST_collection,  result_save_path):
    ...
    for date_i in cloud_file:
        ...
        raster_file = result_save_path + '\\' + str(date_i) + '_LST_M6.tif'  #=============================????????
