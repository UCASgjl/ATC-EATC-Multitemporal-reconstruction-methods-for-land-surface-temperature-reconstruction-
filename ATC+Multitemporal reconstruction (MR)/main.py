...
# CreateTime : 2023/8/4
# FileName : main
# Description : ATC+MR method
from img_class import *
from similar_pixel_research import *

# ======================================================= 创建影像对象 =======================================================
input_cloud_file = "E:\\cloudy"
input_clear_file = "E:\\clear"
input_ATC_file = "E:\\ATC"
...
# 3、ATC拟合LST，即Modeled IMG
ATC_file_all = os.listdir(input_ATC_file)
ATC_img_collection = {}

# 创建ATC拟合的LST影像对象
for file_i_new in ATC_file_all:
    if file_i_new.endswith('.tif'):

        file_new = file_i_new[0:3]

        if os.path.exists(input_ATC_file + "\\" + file_i_new):  # 判断是LE07还是LC08，以此按顺序打开影像
            ATC_img_collection['img' + str(file_new)] = ATC_IMG(input_ATC_file, file_i_new)  # 批量创建影像对象

        # print(clear_img_collection['img' + str(file_i_new)].date)
        # print(ATC_img_collection.keys())

print('ATC Img collection have been created!')
...
for file_i_cloud in cloud_file_all_new:
    if file_i_cloud.endswith('.tif'):
        ...
        # ATC拟合LST
        file_ATC_modeled = get_d_ndvi(file_i_clear)
        file_i_ATC_modeled = f"{file_ATC_modeled:03}"
        print(file_i_ATC_modeled)
        ATC_array = ATC_img_collection['img' + str(file_i_ATC_modeled)].vt_array

        # NDVI影像
        file_i_ndvi = file_i_cloud[:8]
        NDVI_array = NDVI_img_collection['img' + str(file_i_ndvi) + '.tif'].ndvi_array  * 0.0001

        print(file_i_cloud, file_i_clear, file_i_ATC_modeled)

        # 计算 LST
        file_i_LST = file_i_cloud[:8]
        Result_img_collection['img' + str(file_i_LST)].Result_LST_array= LST_calculate(file_i_LST, Cloud_array, ATC_array, NDVI_array, Clear_array, Min_window_size, Max_window_size, Expected_similar_pixels)
